...
module Plot exposing( Graph, size, ranges, make, draw, getSize
                    , Msg, Model, init, update, view
...
update get msg model =
  let model' =
    if msg.id == model.id then updateMouse get msg.mouse model
    else model
  in
    model' ! []
...
view lift (e11,e12s,e22,tmini) model =
  let
    width = Element.widthOf e11 + Element.widthOf e12
    e1h = Element.heightOf e12
    e2h = Element.heightOf e22
    
    e = div [] [e1,e2]

    e1 = Flow.right width e1h [ Element.toHtml e11, graph e12 ]

    e2 = Flow.left width e2h [ Element.toHtml e22 ]
    ...
    e12 = (mouseNotifier ++ e12s) |> Element.layers
...
draw graph =
  let
    ...
    e12s = revMap (singleton width height) marea
...
getSize (e11,e12s,e22,tmini) =
  let
    w = Element.widthOf e11 + e12w
    h = e12h + Element.heightOf e22
    (e12w,e12h) = case e12s of
      e12::_ -> (Element.widthOf e12,Element.heightOf e12)
      [] -> (0,0)
  in
    (w,h)
...
vbounds graph =
  let
    t1 = toString graph.plot.xAxis.modelMax
          |> Text.fromString
          |> Text.height graph.tmini
          |> Element.rightAligned

    t3 = toString graph.plot.xAxis.modelMin
          |> Text.fromString
          |> Text.height graph.tmini
          |> Element.rightAligned

    e12 = Element.layers
            [ Element.container graph.ex graph.plot.height Element.topRight t1
            , Element.container graph.ex graph.plot.height Element.bottomRight t3
            ]
  in
...
hbounds graph =
  let
    t1 = toString graph.plot.tAxis.modelMin
          |> Text.fromString
          |> Text.height graph.tmini
          |> Element.centered
          
    t3 = toString graph.plot.tAxis.modelMax
          |> Text.fromString
          |> Text.height graph.tmini
          |> Element.centered
          
    e22 = Element.layers
            [ Element.container graph.plot.width graph.ex Element.topLeft t1
            , Element.container graph.plot.width graph.ex Element.topRight t3
            ]
  in
...
piece graph p = { graph | marea = p :: graph.marea }
...
scatter glyph pts graph =
  let
    trans pt = glyph graph.gsize |> C.move (graph.plot.toView pt)
  in
    List.map trans pts
...
eval color n f graph =
  let
    range = (graph.plot.tAxis.modelMin,graph.plot.tAxis.modelMax)
...
horiz graph =
  List.map graph.plot.toView
    [(graph.plot.tAxis.modelMin,0),(graph.plot.tAxis.modelMax,0)]
  |> C.path
...
vert graph = 
  List.map graph.plot.toView
    [(0,graph.plot.xAxis.modelMin),(0,graph.plot.xAxis.modelMax)]
  |> C.path
  |> C.traced (solid black)
  |> piece graph
...
dot' color r = C.circle r |> C.outlined (solid color)

sqr' color r = C.square (2*r) |> C.outlined (solid color)

dia' color r = C.square (2*r) |> C.outlined (solid color) |> C.rotate (degrees 45)
